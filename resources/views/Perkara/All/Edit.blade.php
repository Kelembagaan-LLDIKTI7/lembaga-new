...
@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                ...
                <form id="formPerkaraPT" action="{{ route('perkara.update', $perkara->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
                    <input type="hidden" name="id" value="{{ $perkara->id }}">
                    <div class="form-container">
                        <div class="form-left">
                            <div class="mb-3">
                                <label for="title">Judul Perkara</label>
                                <input type="text" name="title" id="title" class="form-control" required
                                    value="{{ old('title', $perkara->title) }}">
                                @error('title')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="tanggal_kejadian" id="tanggal_kejadian" class="form-control"
                                    required value="{{ old('tanggal_kejadian', $perkara->tanggal_kejadian) }}">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="deskripsi_kejadian" id="deskripsi_kejadian" class="form-control" required>{{ old('deskripsi_kejadian', $perkara->deskripsi_kejadian) }}</textarea>
    ...
    <input type="file" name="bukti_foto[]" id="bukti_foto" class="form-control" multiple accept="image/*">
    @error('bukti_foto.*')
        <div class="text-danger">{{ $message }}</div>
    @enderror
...
@section('js')
    ...
    function displayErrors(errors) {
        ...
        for (let field in errors) {
            ...
            $(`#error-${field}`).text(errorMessages);
        }
    }

    function submitToStore(formData) {
        $.ajax({
            url: '{{ route('perkara.update', $perkara->id) }}', // Update route
