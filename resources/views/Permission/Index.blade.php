    ...
    <div class="main-content">
        <div class="page-content">
            <div class="container-fluid">
                <h4 class="title mb-4">Permission Role User</h4>
                                    ...
                                    <table id="dom_jq_event"
                                        ...
                                        <tbody>
                                            @foreach ($permission as $item)
                                                ...
                                                <div class="modal fade" id="editPermissionModal-{{ $item->id }}"
                                                    tabindex="-1"
                                                    aria-labelledby="editPermissionModalLabel-{{ $item->id }}"
                                                    aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title"
                                                                    id="editPermissionModalLabel-{{ $item->id }}">Edit
                                                                    Permission Role User</h5>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <div class="modal-body">
                                                                <form action="{{ route('permission.update', $item->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    <div class="mb-3">
                                                                        <label for="editPermissionName-{{ $item->id }}"
                                                                            class="form-label">Nama Permission</label>
                                                                        <input type="text"
                                                                            class="form-control @error('name') is-invalid @enderror"
                                                                            id="editPermissionName-{{ $item->id }}"
                                                                            name="name"
                            ...
                            <form action="{{ route('permission.store') }}" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                                        id="addPermissionName" name="name" value="{{ old('name') }}"
                                        placeholder="Ex: Create User" required>
                                    @error('name')
