...
@section('content')
    <div class="container-fluid">
        <a href="{{ route('perguruan-tinggi.show', ['id' => $pimpinan->id_organization]) }}"><i
                class="fas fa-arrow-left mb-4 me-2"></i> Kembali
        </a>
        <div class="row">
            <div class="col-12">
                <form id="formPimpinanPTedit" action="{{ route('pimpinan-perguruan-tinggi.update', $pimpinan->id) }}"
                    method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT') <!-- Menentukan method PUT untuk update -->
                    <input type="hidden" name="id_organization" value="{{ $pimpinan->id_organization }}"
                        class="form-control" required>
                    <div class="card bordered">
                        <div class="card-body">
                            <h5 class="card-title mb-4">Form Edit Pimpinan</h5>
                            <div class="row">
                                <div class="col-md-6">
...
@section('js')
    <script>
        $(document).ready(function() {
            $('#loading').hide();
            $('#formPimpinanPTedit').on('submit', function(event) {
                ...
                $.ajax({
                    url: '{{ route('pimpinan-perguruan-tinggi.validationUpdate', ['id' => $pimpinan->id]) }}',
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            function submitToStore(formData) {
                $.ajax({
                    url: '{{ route('pimpinan-perguruan-tinggi.update', ['id' => $pimpinan->id]) }}',
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
