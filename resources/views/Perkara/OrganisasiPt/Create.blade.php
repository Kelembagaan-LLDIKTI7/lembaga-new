...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <form id="formPerkaraPT" action="{{ route('evaluasi-organisasipt.store') }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id_organization" value="{{ $organisasi->id }}" class="form-control"
                        required>

                    <div class="card bordered">
                        <div class="card-body">
                            <h5 class="card-title mb-4">Tambah Evaluasi Perguruan Tinggi</h5>
                            <div class="row">
                                <div class="col-md-6">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#formPerkaraPT').on('submit', function(event) {
                ...
                $.ajax({
                    url: '{{ route('evaluasi-organisasipt.validationStore') }}',
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            function submitToStore(formData) {
                $.ajax({
                    url: '{{ route('evaluasi-organisasipt.store') }}',
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
