...
@section('content')
    ...
    @method('PUT')
                    ...
                    <input type="hidden" name="redirect_route" value="{{ route('evaluasi.showprodi', ['id' => $perkaras->id]) }}">
...
@section('js')
    ...
    for (let field in errors) {
        const errorElement = $(`#error-${field.replace('.*', '')}`); // Tangani array seperti bukti_foto.*
    ...
    function submitToStore(formData) {
        $.ajax({
            url: '{{ route('evaluasi.updateprodi', $perkaras->id) }}', // Update route
